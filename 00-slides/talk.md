...
* use `packageVersion` to check for package versions if needed.

* Don't modify `options`, if you must do so
...
* From `vignette("programming", package = "dplyr")`: dplyr code is ambiguous.
  Depending on what variables are defined where, filter(df, x == y) could be
...
* Several times I would `fetch` changes that would not run.

    * development work pollutes your workspace.

    * Code might work in your current interactive session but not in a new session.


```r
# What happens when these two lines are evaluated twice in one session?
x + 2
x <- 2
```
...
* I'm sitting in the developer bullpen.  Tester comes running down form the
  testing office and had some choice words for me; delivered loudly and publicly.
...
* The tester was testing TWO things
    ...
    1. When inputting 73 for height and 300 for weigth, does the function return
       a value of 39.6?
...
* Supervisor drops by and asks for a "nice-to-have" feature.  I tell him
...
* Developers and testers work to find the offending code.  It was found in the
...
* There was no meaningful version control.  No way to show who had made the
  error, nor to revert the system.  
...
* This event was cited as the primary reason for not being promoted during my
  annual review two months later.
...
* github.com:
    ...
    - add a `.travis.yaml` file to the root of your project working directory.
